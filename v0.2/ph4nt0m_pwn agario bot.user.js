// ==UserScript==
// @name        ph4nt0m_pwn agario bot logic
// @namespace   ph4nt0m_pwn agario bot logic
...
function getLatestCommit() {
    window.jQuery.ajax({
            url: "https://api.github.com/repos/OmniPhaser/Agar.io-bot/git/refs/heads/master",
            cache: false,
            dataType: "jsonp"
        }).done(function(data) {
            ...
            $.get('https://raw.githubusercontent.com/OmniPhaser/Agar.io-bot/master/bot.user.js?' + Math.floor((Math.random() * 1000000) + 1), function(data) {
                ...
                {
                    confirm("This script is outdated. Please contact the developer.");
                    //update("aposBot", "bot.user.js", "https://github.com/OmniPhaser/Agar.io-bot/blob/" + sha + "/bot.user.js/");
                }
...
(function(f, g) {
    ...
    f.botList.push(["ph4nt0m_pwn-bot " + aposBotVersion, findDestination]);

    var bList = g('#bList');
    g('<option />', {value: (f.botList.length - 1), text: "ph4nt0m_pwn-bot"}).appendTo(bList);
